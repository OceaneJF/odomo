...
class Meteo {
    ...
    /**
     * Hygrométrie par minute, pour les 60 dernière minutes
     */
    ...
    /**
     * Hygrométrie par jour, pour les 365 derniers jours
     */
    ...
    static void finMinuteTemperExtMoy() {
        double somme = 0.;
        for (double temperExtMoyMinut : temperExtMoyMinute) {
            somme += temperExtMoyMinut;
        }
        double moyenneMinutes = somme / temperExtMoyMinute.length;
        temperExtMoyHeure[0] = moyenneMinutes;
        somme = 0.;
        for (double temperExtMoyHeur : temperExtMoyHeure) {
            somme += temperExtMoyHeur;
        }
        double moyenneHeures = somme / temperExtMoyHeure.length;
        temperExtMoyJour[0] = moyenneHeures;
    ...
    static boolean temperaturesCoherentes() {
        boolean temperatureCoherentes = true;
        int i = 0;
        while (temperatureCoherentes && i < 60) {
            if (temperExtMinMinute[i] > temperExtMoyMinute[i] || temperExtMoyMinute[i] > temperExtMaxMinute[i]) {
                temperatureCoherentes = false;
            }
            i++;
        }
        int j = 1;
        while (temperatureCoherentes && j < 60) {
            if (temperExtMinMinute[j] < temperExtMinHeure[0] || temperExtMaxHeure[0] < temperExtMaxMinute[j]) {
                temperatureCoherentes = false;
            }
            j++;
        }
        return temperatureCoherentes;
    }
    ...
    static boolean[][] matricePluvioHeure() {
        ...
        double pluviometrieMax = valeurMax(pluvioHeure);
        for (int i = 0; i < 24; i++) {
            double pluvioH = pluvioHeure[23 - i];
            if (pluvioH == 0) {
                nbCases = 0;
            } else {
                nbCases = 8 - Ecran.numLigne(pluvioH, 0, pluviometrieMax);
            }
            allumerNbCasesDuBas(matricePluvio, i, nbCases);
        }
    ...
    static boolean[][] matriceTemperMinutes() {
        ...
        for (int i = 0; i < 24; i++) {
            int[] indices = agreger60vers24(i);
            double[] valeursMin = tableauDepuisIndices(temperExtMinMinute, indices);
            double[] valeursMax = tableauDepuisIndices(temperExtMaxMinute, indices);
            double valeurMax = valeurMax(valeursMax);
            double valeurMin = valeurMin(valeursMin);
            int ligneBas = Ecran.numLigne(valeurMin, valeurMin(temperExtMinMinute), valeurMax(temperExtMaxMinute));
            int ligneHaut = Ecran.numLigne(valeurMax, valeurMin(temperExtMinMinute), valeurMax(temperExtMaxMinute));
    ...
    static int[] agreger60vers24(int col) {
        ...
        if (col % 2 == 0) {
            tabAgrege = new int[]{valeurDepart, valeurDepart + 1};
        } else {
            tabAgrege = new int[]{valeurDepart, valeurDepart + 1, valeurDepart + 2};
        }
        return tabAgrege;
    }

// Dexième facon possible d'implémenter la méthode agreger60vers24(): 
//    static int[] agreger60vers24(int col) {
//        int valeurDepart = 60;
//        int[][] tabAgrege = new int[24][];
//        for (int i = 0; i < 24; i++) {
//            if (i % 2 == 0) {
//                valeurDepart -= 2;
//            } else {
...
//            }
//            if (col % 2 == 0) {
//                tabAgrege[i] = new int[]{valeurDepart, valeurDepart + 1};
//            } else {
//                tabAgrege[i] = new int[]{valeurDepart, valeurDepart + 1, valeurDepart + 2};
//            }
//        }       
//        return tabAgrege[col];
//    } 
    ...
    static double[] tableauDepuisIndices(double[] tab, int[] indices) {
        double[] nouvTab = new double[indices.length];
        for (int i = 0; i < nouvTab.length; i++) {
            nouvTab[i] = tab[indices[i]];
        }
        return nouvTab;
    }
    ...
    static void allumerNbCasesIntervalle(boolean[][] matrice, int colonne,
        ...
        for (int i = 0; i < matrice.length; i++) {
            matrice[i][colonne] = i >= ligneHaut && i <= ligneBas;
        }
    }
